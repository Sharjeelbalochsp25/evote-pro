...
if (autoStash && isDirty) {
  if (releaseScriptDirty) {
    log('release script is dirty, continuing through the temp-copy path');
  }
...
const env = { ...process.env, ALLOW_AUTO_STASHED_RELEASE: stashCreated ? '1' : '0', NO_AUDIT: stashCreated ? '1' : (process.env.NO_AUDIT || '0') };
